...
export default function AdminPage() {
  ...
  useEffect(() => {
    void api<{ csrf: string }>('/api/admin/session').then(result => {
      setCsrf(result.csrf); setScreen('ready'); return loadPosts(result.csrf);
  ...
  if (screen === 'login') return <Login onLogin={token => { setCsrf(token); setScreen('ready'); void loadPosts(token); }} />;
