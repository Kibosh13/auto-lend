...
- Secrets: `/etc/ngreport.env`, root-owned mode 600, read by systemd.
...
- Telegram does not send ordinary channel-post deletion events; deleting a post
  in Telegram does not remove the stored website copy. Site-side removal requires
  an explicit maintenance action. The service does not create test posts.
...
`curl http://127.0.0.1:3101/health`, the public page and `/api/posts`.
...
rotating the Telegram token, update `/etc/ngreport.env` and restart only the feed
service. No password or token is recorded in this repository.
